...
// Package cli provides a command-line interface (CLI) implementation of the UI interface.
package cli
...
func (c *CLI) PrintInventoryVars(i *inventory.Inventory) {
	...
	for name, host := range i.Hosts() {
		...
		for key, value := range host.Vars() {
			...
			sb.WriteString(hclutil.FormatCtyValueToIndentedString(value, 4, 4))
			if c.color {
				sb.WriteString("\033[0m") // Reset
			}
...
func (c *CLI) printText(writer io.Writer, text string) {
	...
	text = secret.SecretFilter.Filter(text)
...
func (c *CLI) printResult(label string, r result.Result) {
	...
	stdout := secret.SecretFilter.Filter(outStringBuilder.String())
	stderr := secret.SecretFilter.Filter(errorStringBuilder.String())
