...
func (e ExpectedHostOutput) Verify(
	...
	if !ok {
		...
		errorMsg, hasError := hostOutput["error"]
		errorDetail, hasErrorDetail := hostOutput["error_detail"]
		if hasError {
			if hasErrorDetail {
				t.Errorf(
					"Expected host %q in step %q in process %d failed value to be %t, got %t: %s; %s",
					hostName,
					stepName,
					processIndex,
					e.Failed,
					failed.True(),
					errorMsg.AsString(),
					errorDetail.AsString(),
				)
			} else {
				t.Errorf(
					"Expected host %q in step %q in process %d failed value to be %t, got %t: %s",
					hostName,
					stepName,
					processIndex,
					e.Failed,
