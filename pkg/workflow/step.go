...
func (s *SingleStep) runOnHost(hwc *HostWorkflowContext) (cty.Value, error) {
	...
	if s.common.condition != nil {
		...
		condition, diags = hclutil.ConvertHCLAttributeToBool(s.common.condition, hwc.evalContext)
		if diags.HasErrors() {
...
func (s *SingleStep) formatResultOutput(result *result.Result) cty.Value {
	...
	if result.Error != nil {
		outputMap["error"] = cty.StringVal(result.Error.Error())
		outputMap["error_detail"] = cty.StringVal(result.ErrorDetail)
	}
...
func (s *SingleStep) runHostIteration(hwc *HostWorkflowContext, iteration *StepIteration) (cty.Value, error) {
	...
	if !iteration.index.IsNull() && !iteration.value.IsNull() {
		...
		if s.common != nil && s.common.loop != nil && s.common.loop.label != nil {
			iteration.label, diags = hclutil.ConvertHCLAttributeToString(s.common.loop.label, hwc.evalContext)
			if diags.HasErrors() {
		...
		if s.common != nil && s.common.loop != nil && s.common.loop.condition != nil {
			condition, diags = hclutil.ConvertHCLAttributeToBool(s.common.loop.condition, hwc.evalContext)
			if diags.HasErrors() {
				output := result.NewFailure(diags, diags.Error())
				return s.handleHostIterationResult(hwc, iteration, output), diags
			}
	...
	if s.common != nil && s.common.execTimeout != nil {
		var diags hcl.Diagnostics
		timeout, diags = hclutil.ConvertHCLAttributeToDuration(s.common.execTimeout, hwc.evalContext)
		if diags.HasErrors() {
			result := result.NewFailure(diags, diags.Error())
	...
	if s.common != nil && s.common.whatIf != nil {
		var diags hcl.Diagnostics
		whatIf, diags = hclutil.ConvertHCLAttributeToBool(s.common.whatIf, hwc.evalContext)
		if diags.HasErrors() {
			result := result.NewFailure(diags, diags.Error())
			return s.handleHostIterationResult(hwc, iteration, result), diags
		}
	...
	if s.output.failedCondition != nil {
		var diags hcl.Diagnostics
		result.Failed, diags = hclutil.ConvertHCLAttributeToBool(s.output.failedCondition, hwc.evalContext)
		if diags.HasErrors() {
			result.Failed = true
			result.Error = errors.Join(result.Error, diags)
		}
	}

	if s.output.changedCondition != nil {
		var diags hcl.Diagnostics
		result.Changed, diags = hclutil.ConvertHCLAttributeToBool(s.output.changedCondition, hwc.evalContext)
		if diags.HasErrors() {
	...
	if s.output.continueOnFail != nil {
		var diags hcl.Diagnostics
		continueOnFail, diags = hclutil.ConvertHCLAttributeToBool(s.output.continueOnFail, hwc.evalContext)
		if diags.HasErrors() {
			continueOnFail = false
			result.Failed = true
			result.Error = errors.Join(result.Error, diags)
...
func (s *SingleStep) getEscalation(hwc *HostWorkflowContext) (*transport.Escalation, error) {
	...
	escalate, diags := hclutil.ConvertHCLAttributeToBool(s.escalate.escalate, hwc.evalContext)
	if diags.HasErrors() {
		return nil, diags
	}
	...
	if s.escalate.password != nil {
		p, diags := hclutil.ConvertHCLAttributeToString(s.escalate.password, hwc.evalContext)
		if diags.HasErrors() {
			return nil, diags
		}
	...
	impersonateUser, diags := hclutil.ConvertHCLAttributeToString(s.escalate.impersonateUser, hwc.evalContext)
	if diags.HasErrors() {
		return nil, diags
	}
